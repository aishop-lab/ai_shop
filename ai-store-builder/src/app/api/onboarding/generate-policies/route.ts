...
export async function POST(request: Request) {
    try {
        ...
        const { data: store, error: storeError } = await supabase
            .from('stores')
            .select('id, name, owner_id, settings, blueprint')
            .eq('id', store_id)
            .eq('owner_id', user.id)
            .single()
        ...
        console.log('[GeneratePolicies] Generating policies for store:', store.name)

        // Extract store info for policy generation
        const category = store.blueprint?.category?.primary || 'General'
        const freeThreshold = store.settings?.shipping?.free_shipping_threshold || 999
        const flatRate = store.settings?.shipping?.flat_rate_national || 49
        const codEnabled = store.settings?.shipping?.cod_enabled || false

        // Generate policies using AI
        const now = new Date().toISOString()

        // Generate Return Policy
        const returnPolicyContent = generateReturnPolicy(store.name)

        // Generate Privacy Policy
        const privacyPolicyContent = generatePrivacyPolicy(store.name)

        // Generate Terms of Service
        const termsContent = generateTermsOfService(store.name)

        // Generate Shipping Policy
        const shippingPolicyContent = generateShippingPolicy(
            store.name,
            freeThreshold,
            flatRate,
            codEnabled
        ...
        // Update store with generated policies
        ...
        if (updateError) {
            console.error('[GeneratePolicies] Failed to save policies:', updateError)
            return NextResponse.json(
                { success: false, error: 'Failed to save policies' },
                { status: 500 }
            )
        }

        console.log('[GeneratePolicies] Policies saved successfully')

        return NextResponse.json({
            success: true,
            message: 'Legal policies generated successfully',
        ...
        return NextResponse.json(
            { success: false, error: 'Failed to generate policies' },
            { status: 500 }
        )
    }
}

// Helper functions to generate policy content
